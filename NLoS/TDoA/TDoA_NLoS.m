...
% UAVTag 객체 생성
tag1 = UAVTag_TDoA(1, Scenario, initialPosition1, waypoints1, timeOfArrival1);
tag2 = UAVTag_TDoA(2, Scenario, initialPosition2, waypoints2, timeOfArrival2);
...
figure('Name', 'TDoA NLoS 위치 추정 오차');
for i = 1:length(tags)
    ...
    title(sprintf('태그 %d - TDoA NLoS 위치 추정 오차', tags(i).ID));
    xlabel('시간 (초)');
    ylabel('오차 (m)');
    grid on;
end
...
figure('Name', '하이브리드 TDoA NLoS 위치 추적 결과');
hold on;
...
for i = 1:length(tags)
    if ~isempty(tags(i).TDoAData.EstimatedTime) && ~isempty(tags(i).TDoAData.TagPosition)
        ...
        plot3(tags(i).TDoAData.EstimatedPosition(:,2), tags(i).TDoAData.EstimatedPosition(:,1), -tags(i).TDoAData.EstimatedPosition(:,3), ...
              [colors{i}, '--'], 'LineWidth', 1.5, 'DisplayName', sprintf('태그 %d TDoA NLoS 추정 경로', tags(i).ID));
...
ylabel('북쪽 (m)'); 
zlabel('높이 (m)');
title('TDoA NLoS 기반 다중 UAV 위치추적 결과');
