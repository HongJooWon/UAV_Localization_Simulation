classdef UAVTag_ParticleTWR < handle
    ...
    methods
        function obj = UAVTag_ParticleTWR(id, scenario, initialPosition, waypoints, timeOfArrival)
            ...
            for i = 1:obj.NumParticles
                ...
                for a = 1:length(distances)
                    if distances(a) > 0 % 측정된 거리가 있는 경우만
                        ...
                        % 우도 계산 (가우시안 노이즈 가정)
                        dist_likelihood = exp(-0.5 * (dist_error / 0.1)^2); % 10cm 표준편차 가정
        ...
        function plotResults(obj, anchorPositions)
            ...
            if obj.ParticleFilterActive
                title(sprintf('태그 %d - 파티클 필터 TWR 기반 UAV 위치추적 결과', obj.ID));
            else
        ...
        function plotParticles(obj, anchorPositions)
            % 현재 파티클 분포를 3D로 시각화
            %
            % 입력:
            %   anchorPositions - 앵커 위치 행렬
            ...
            figure('Name', sprintf('태그 %d - 파티클 분포', obj.ID));
            ...
            grid on;
            xlabel('동쪽 (m)'); ylabel('북쪽 (m)'); zlabel('높이 (m)');
            title(sprintf('태그 %d - 파티클 필터 TWR 위치 추정', obj.ID));
            legend('show');
            axis equal;
            xlim([0 15]);
            ylim([0 15]);
        ...
        function plotCombinedResults(obj, otherTags, anchorPositions)
            ...
            grid on;
            xlabel('동쪽 (m)'); ylabel('북쪽 (m)'); zlabel('높이 (m)');
            title('파티클 필터 TWR 기반 다중 UAV 위치추적 결과');
